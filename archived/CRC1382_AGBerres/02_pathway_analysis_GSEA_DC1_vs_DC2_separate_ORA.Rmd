...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=20, fig.height=12}
headings <- hash()
headings[["all_LV_and_all_PV"]] <- "All LV and all PV samples"
headings[["all_LV_samples"]] <- "All LV samples"
headings[["all_PV_samples"]] <- "All PV samples"
headings[["LV20"]] <- "LV20"
headings[["LV21"]] <- "LV21"
headings[["PV20"]] <- "PV20"
headings[["PV21"]] <- "PV21"
...
compare_DC1_DC2_raw[["all_LV_and_all_PV"]] %>% create_dt()
```

```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=20, fig.height=12}
for (compare.case in names(compare_DC1_DC2_raw)){
  cat(paste("\n\n### ", headings[[compare.case]], "##\n"))
...
## GO-ORA for up-in-DC1 genes only {.tabset}
...
for (compare.case in names(compare_DC1_DC2_raw)){
  cat(paste("\n\n### ", headings[[compare.case]], "##\n"))
  tmp.full.list <- compare_DC1_DC2_raw[[compare.case]]
  input.gene.list <- subset(tmp.full.list, tmp.full.list$p_val_adj <= 0.05 & avg_log2FC >= 0)$Gene
  ...
  if (length(input.gene.list != 0)){
    ...
    if (compare.case == "all_PV_samples"){
      enrich.plot <- enrichplot::dotplot(ora.GO)
      plotdf <- subset(ora.GO, ora.GO$ID %in% c("GO:0032695", "GO:0032753"))
      selected.pathway.plot <- plotdf %>% ggplot(aes(x = GeneRatio, y = Description)) + 
        geom_point(aes(color = p.adjust, size = Count)) + scale_fill_gradient(low = "blue", high = "red")
      demo.selected.pathway.plot <- enrichplot::dotplot(ora.GO)
    }
...
## GO-ORA for up-in-DC2 genes only {.tabset}
...
## KEGG-ORA for up-in-DC1 genes only {.tabset}
...
## KEGG-ORA for up-in-DC2 genes only {.tabset}
