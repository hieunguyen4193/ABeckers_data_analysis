...
for (cluster.id in sort(unique(s.obj@meta.data$seurat_clusters))){
  ...
  if (nrow(tmp.cluster.surface_markers) == 0){
    ...
    p <- FeaturePlot(object = s.obj, reduction = "RNA_UMAP", features = head(tmp.cluster.surface_markers, 12)$gene, ncol = 3, label = TRUE, pt.size = 0.5, label.size = 5, label.color = "red")    
  }
  print(p)
  cat("\n \n")
}
...
for (g in intersect(unique(marker.genedf$Gene), row.names(s.obj))){
  ...
  DefaultAssay(s.obj) <- "RNA"
  umap.plot <- FeaturePlot(object = s.obj, reduction = "RNA_UMAP", features = c(g), label = TRUE, pt.size = 1.5, slot = "data")
