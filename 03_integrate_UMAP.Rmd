---
title: "Integrate old and new datasets, `r params$output.version`, cluster.resolution `r params$cluster.resolution`"
author:
...
s.obj.batch1 <- readRDS(file.path(outdir, PROJECT, output.version, "batch1", sprintf("res_%s", cluster.resolution), "data_analysis", "01_output", "batch1.rds"))
s.obj.batch2 <- readRDS(file.path(outdir, PROJECT, output.version, "batch2", sprintf("res_%s", cluster.resolution), "data_analysis", "01_output", "batch2.rds"))
```
...
DimPlot(object = s.obj.batch2, reduction = "INTE_UMAP", label = TRUE, label.box = TRUE, repel = TRUE)
```

## Previous dataset

```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.height=10, fig.width=14}
DimPlot(object = s.obj.batch1, reduction = "INTE_UMAP", label = TRUE, label.box = TRUE, repel = TRUE)
```
...
if (file.exists(file.path(path.to.03.output, "merge_all_6_samples.RNA_assay.rds")) == FALSE){
  ...
  s.obj <- FindClusters(s.obj, resolution = 0.5, random.seed = chosen.seed)
...
# Some marker genes, grouped by cell type
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.height=10, fig.width=14}
if (output.version == "default"){
  p <- DimPlot(object = s.obj, reduction = "INTE_UMAP", group.by = "celltype", label = TRUE, label.box = TRUE)
  print(p)
}
```
...
if (output.version == "default"){
  for (g in unique(marker.genedf$Gene)){
    dir.create(file.path(path.to.03.output, "marker_gene_plots_grouped_by_annotation"), showWarnings = FALSE, recursive = TRUE)
    DefaultAssay(s.obj) <- "RNA"
    Idents(s.obj) <- "celltype"
    umap.plot <- FeaturePlot(object = s.obj, reduction = "INTE_UMAP", features = c(g), label = TRUE, pt.size = 1.5, slot = "data")
    vln.plot <- VlnPlot(object = s.obj, features = c(g), group.by = "celltype", slot = "data")
    ggsave(plot = umap.plot, filename = sprintf("UMAP_plot_%s.svg", g), path = file.path(path.to.03.output, "marker_gene_plots_grouped_by_annotation"), device = "svg", dpi = 300, width = 14, height = 10)
    ggsave(plot = vln.plot, filename = sprintf("Violin_plot_%s.svg", g), path = file.path(path.to.03.output, "marker_gene_plots_grouped_by_annotation"), device = "svg", dpi = 300, width = 14, height = 10)
  }
  ...
  for (g in all.surface.markers){
    DefaultAssay(s.obj) <- "RNA"
    Idents(s.obj) <- "celltype"
    dir.create(file.path(path.to.03.output, "surface_marker_plots_grouped_by_annotation"), showWarnings = FALSE, recursive = TRUE)
    umap.plot <- FeaturePlot(object = s.obj, reduction = "INTE_UMAP", features = c(g), label = TRUE, pt.size = 1.5, slot = "data")
    vln.plot <- VlnPlot(object = s.obj, features = c(g), group.by = "celltype", slot = "data")
    ggsave(plot = umap.plot, filename = sprintf("UMAP_plot_%s.svg", g), path = file.path(path.to.03.output, "surface_marker_plots_grouped_by_annotation"), device = "svg", dpi = 300, width = 14, height = 10)
    ggsave(plot = vln.plot, filename = sprintf("Violin_plot_%s.svg", g), path = file.path(path.to.03.output, "surface_marker_plots_grouped_by_annotation"), device = "svg", dpi = 300, width = 14, height = 10)
  }
}
...
if (output.version == "default"){
  ...
  for (g in unique(new.markers)){
    Idents(s.obj) <- "celltype"
    dir.create(file.path(path.to.03.output, "alias_marker_gene_plots_grouped_by_annotation"), showWarnings = FALSE, recursive = TRUE)
    DefaultAssay(s.obj) <- "RNA"
    umap.plot <- FeaturePlot(object = s.obj, reduction = "INTE_UMAP", features = c(g), label = TRUE, pt.size = 1.5, slot = "data")
    vln.plot <- VlnPlot(object = s.obj, features = c(g), group.by = "celltype", slot = "data")
    ggsave(plot = umap.plot, filename = sprintf("UMAP_plot_%s.svg", g), path = file.path(path.to.03.output, "alias_marker_gene_plots_grouped_by_annotation"), device = "svg", dpi = 300, width = 14, height = 10)
    ggsave(plot = vln.plot, filename = sprintf("Violin_plot_%s.svg", g), path = file.path(path.to.03.output, "alias_marker_gene_plots_grouped_by_annotation"), device = "svg", dpi = 300, width = 14, height = 10)
  }
}
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.height=10, fig.width=14}
DefaultAssay(s.obj) <- "RNA"
check.genes <- c("HLA-DRB1", "HLA-DRA", "HLA-DRB5", "CLEC10A", "CD301", "CLECSF13", "CLECSF14", "HML", "HML2", "CD117", "KIT", 
                 "C-Kit", "CD117", "PBT", "SCFR")
intersect(row.names(s.obj), check.genes)
```
...
dir.create(file.path(path.to.03.output, "module_scores"), showWarnings = FALSE, recursive = TRUE)
ggsave(plot = feature.plot, filename = sprintf("feature_plot_module_scores.svg"), 
       path = file.path(path.to.03.output, "module_scores"), device = "svg", width = 20, height = 14)
...
ggsave(plot = heatmap.plot, filename = sprintf("heatmap_module_scores.svg"), 
       path = file.path(path.to.03.output, "module_scores"), device = "svg", width = 10, height = 10)
```
